...
import numpy as np
from lry_tools.times.utils import filter_by_time
...
class LSConverter:
    ...
    def convert_to_coco(self, input_json: str, output_json: str):
        ...
        for item_idx, item in enumerate(ann_list):
            ...
            # # 时间过滤
            # created_after = '2025-11-28',  # 11月18日3点之后
            # if updated_author_id is not None and filter_by_time(created_time, updated_time, created_after=created_after):
            #     continue
            ...
            for label in all_labels:
                ...
                if label['type'] == 'rectanglelabels':
                    ...
                    if 'rectanglelabels' not in value_keys:
                        warnings.warn(f"missing rectanglelabels key in value, value_keys: {value_keys}")
                        breakpoint()
            ...
            for label_studio_bbox_id, data in annotations_dict.items():
                ...
                bbox_data = data['bbox']
                if 'value' not in bbox_data:
                    warnings.warn(f'bbox data does not contain key {label_studio_bbox_id}')
                    continue
                bbox_category = bbox_data["value"]['rectanglelabels']
                # 转换bbox
                ...
                for keypoint_ann in keypoint_anns:
                    ...
                    keypoints_item = {'kpt_category_id': category_id, 'keypoints': keypoints}
                    keypoints_list.append(keypoints_item)
                if len(bbox_category) != 1:
                    warnings.warn(f"bbox has more than one category, bbox id: {bbox_ann['id']}")
                    continue
                bbox_category_id = self.rect_name_to_id[bbox_category[0]]
                annotations_coco.append({
                    'id': len(annotations_coco),
                    'image_id': image_id,
                    'category_id': bbox_category_id,
                    'bbox': bbox,
                    'area': bbox[2] * bbox[3],
    ...
    def _get_coco_categories(self):
        ...
        # 多边形类别（关键点）
        for poly_name, poly_id in self.rect_name_to_id.items():
            # 默认关键点命名（可根据实际需求修改）
...
def main():
    config = r"D:\MyProjects\ls_pano.xml"
    input_json = r"D:\dataset\全景251218.json"
    output_json = r"D:\MyProjects\全景251218_bbox.json"
